...
class Catalog {
 public:
  ...
  TableMetadata *CreateTable(Transaction *txn, const std::string &table_name, const Schema &schema) {
    ...
    auto res = table_metadata.get();
    tables_.emplace(res->oid_, std::move(table_metadata));
    names_.emplace(res->name_, res->oid_);
    return res;
  }
  ...
  TableMetadata *GetTable(table_oid_t table_oid) {
    auto oid_it = tables_.find(table_oid);
    if (oid_it == tables_.end()) {
      throw std::out_of_range("cannot find table by table oid");
    }
    return oid_it->second.get();
  }
  ...
  std::vector<IndexInfo *> GetTableIndexes(const std::string &table_name) {
        ...
        for (auto inner_it = inner_map.begin(); inner_it != inner_map.end(); ++inner_it) {
          result.push_back(GetIndex(inner_it->second));
        }
        return result;
  }
